...
hits=$(grep -rnE 'WaitForSingleObject\([^,]*,\s*INFINITE' \
    ...
    | grep -v 'arch-lint: allow-infinite' || true)
if [ -n "$hits" ]; then
...
hits=$(grep -rnE 'HashMap<\(u32,\s*u32\)' $rs_dirs 2>/dev/null || true)
if [ -n "$hits" ]; then
...
hits=$(grep -rnE '\b(MainToChild|ChildToMain)\b' --include='*.rs' \
    common daw_gui daw_audio daw_plugin_host ui 2>/dev/null || true)
if [ -n "$hits" ]; then
...
#    baseline: ClipContent の 1 箇所 (S5 で tag 化されたら 0 に更新)。
untagged_baseline=1
n=$(grep -rn '#\[serde(untagged)\]' --include='*.rs' common/src 2>/dev/null | wc -l)
if [ "$n" -gt "$untagged_baseline" ]; then
    warn UNTAGGED "serde(untagged) が baseline($untagged_baseline) を超過 ($n)。新規 untagged enum を作らない:"
    grep -rn '#\[serde(untagged)\]' --include='*.rs' common/src
fi

# 5. protocol への bulk blob 直載せ (不変条件 2)。
hits=$(grep -nE 'Vec<f32>|Arc<\[u8\]>' common/src/protocol.rs 2>/dev/null || true)
if [ -n "$hits" ]; then
...
hits=$(grep -rnE 'ArrangementEditRequest|split_into_morae|Edit::Undoable' \
    --include='*.rs' ui/crates/ui/src 2>/dev/null || true)
if [ -n "$hits" ]; then
