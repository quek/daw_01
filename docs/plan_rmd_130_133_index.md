...
herdr の claude 起動は auto mode の classifier に拒否されたので、実装は Workflow の並列 agent (1 項目 1 agent +
独立レビュー agent) で行った。実際の統合順は #133 → #132 → #131 → #130 (予定どおり)。#130 と #132 が発音台帳を
別々に作ったので、#132 の `daw_audio/src/note_ledger.rs` 1 本に寄せて #130 を載せ替えた。

### 実装中にユーザーが決めたこと
- #130 トランスポートの幅: **今のまま** (既定の窓幅 1280px では右端の Launch 量子化が Panic に重なる。窓を広げれば解消)。
- Text クリップ (VOICEVOX 読み上げ) の分割: **読み上げは最初の片だけ、続きの片は字幕だけ**。
- プラグインを降ろす undo / redo: **先に plugin state を取り寄せてから動かす** (その Ctrl+Z だけ反映が遅れる。連打は順番に処理)。

### ユーザー不在の間に main が決めたこと (理想の側を選んだ。変えたければ差し戻す)
- 続きの Text 片はクリップに「続き」チップを出し、Inspector に「ここから読む」トグルを付ける。
- 分割でできたクリップへの Inspector 編集 / Auto-Fade / Auto-Crossfade は、そのクリップの窓に見えている片だけに効かせる
  (分割直後に掛けても、分割前に掛けてから割ったのと同じ結果)。
- Melodyne (ARA) は分割した片で modification を共有し、persistent id を安定 id にして旧アーカイブは restore filter で読み替える。
- 未保存プロジェクトの取り込み / bounce キャッシュは文書ごとの置き場に分け、保存 / 閉じる / 破棄 / 起動時に掃除する。
- 録音 take の最中に始まった gesture (ツマミ / 数値欄 / カラーピッカー) は take の undo step に入る。
- 読み込み中に押した書き出し / 解析 / Bounce / Glue は断らず、読み込みの確定を待って自動で始める (キャンセル可)。
- 読み込み中の device だけを降ろす undo / redo は state を取り寄せない (host にまだ居ないので失う値が無い)。

### r.md 4 項目の外で直した同じ根の欠陥
- 再生中にノートの音程を変える / 割る / 短くする / 消すと旧鍵盤が停止まで鳴り残る (台帳で送った鍵盤を消す)。
- 同時に鳴るノートの note_id 衝突 (16384 で畳む導出)、linked clip 2 本を同時表示したノート編集の二重適用。
- ピアノロールの E / J と view からの Song 編集が `handle_event` を通らず undo step が崩れる (全経路を AppEvent へ)。
- 有効に戻したトラックがプラグインの読み込み完了前に FX 無しの音で鳴る (読み込み中は engine のグラフから外す)。
- 分割で audio event の音が変わる (take 窓)、クロスフェードの張り出しの二重再生、Bounce In Place が共有 content を丸ごと置換。
- 取り込みキャッシュを最終名へ直接書いて並行取り込みが書きかけを読む (atomic 書き込み)、テストがユーザーの実データへ書く。
- 未保存の 2 文書が同じキャッシュを共有し、片方の保存でもう片方の素材が消える。
- プラグインを降ろす undo でツマミの値が消える、録音 take 中のツマミで take が割れる。
