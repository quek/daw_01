...
| Q10  | ランチャーの状態 (主導権 + 鳴っているセル) は **`.daw` に保存**する。ただし撃つ / 止めるは「聴き方」なので **`*` も undo も付けない** (2026-09-03 改訂、§1.3)        |
...
`scenes` / `session_clips` / `launch` / `follow`。

**再生状態** (`Track.launcher` / `AutomationLane.launcher` / `last_launched_scene_id`):
`.daw` に保存はする (停止 → 再生で同じセルが鳴り直す、書き出しに反映する = Q9 / Q10) が、
撃つ / 止める / アレンジへ返すは「聴き方」なので **`*` を立てず undo にも積まない**
(2026-09-03 改訂。それまでは「変更で `*` が立つ」だった)。書く口は `AppData::edit_playback`
(`SongDoc::edit_playback`) だけで、`edit_song` と違い `edit_epoch` (dirty / 履歴) を進めず
`sync_epoch` (子プロセス sync) だけ進める。undo / redo は差し替えた Song に**今の**再生状態を
持ち越す (`Song::carry_playback_state_from`) ので、undo でセルが止まったり鳴り出したりしない。
...
| いつ | Song を書き換えるか | dirty / undo |
|---|---|---|
| ユーザーがセル / シーンを撃つ・行を止める・アレンジへ返す | **書き換える** (`edit_playback`) | 付けない (§1.3) |
| フォローアクションで次のセルへ移る | **書き換えない** (engine の走行状態のみ) | 付けない |
