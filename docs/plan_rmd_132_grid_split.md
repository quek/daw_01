...
| ARA = 1 | persistent id を安定 id に (素材 / content と take と素材 / クリップと event)。 片は modification を共有し、document は差分で編集 (作り直さない)、restore は新しい object にだけ filter で。 destroy する modification は直前にその状態を partial archive に取り、作り直す (undo / redo) ときはそこから戻す。 写した take (Make Unique / 独立な複製 / 貼り付け / 別のプロジェクトから) の modification は、document に初めて現れるとき写した元の編集から始める (v43 で `Song::content_forked_from` を take ごとの `AudioEvent::take_origins` に置き換え、別の document / 取っておいた状態 / クリップボードの写しからも引く、`daw_plugin_host::ara::states`)。 v41 以前のアーカイブは読み替え表 (linked clip は content を分けてクリップごとの編集を保つ) | `common::ara_ids`、`daw_plugin_host::ara::graph_plan` / `session` |
