...
- ✅ Phase 63a 完了 (本 commit、 unit test で popup primitive の clip_rect 経路を保証、 daw_01 daw_gui での #014 修正確認は user 側待ち)
...
- ✅ daw_01 conversation #014 を `[Open]` → `[Replied]` に更新 (本 commit。 popup_layer entry で `current_clip = None` 退避を追加、 popup overlay は base scene の clip 制約から免除される不変条件を gui_01 側で吸収、 daw_01 側コード変更不要 = gui_01 path 依存再ビルドのみで効く)
