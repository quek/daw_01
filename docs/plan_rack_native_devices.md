...
| 同 | `add_native(chain, kind, open_panel)` | (挿す chain は呼び出し側が決める。picker でトラックが 0 本なら `select_plugin_from_db` の `plugin_insert_chain` がトラックを足してから渡す) `edit_song_checked` の closure で `owner = chain_owner_track(chain)?`、`id = alloc_device_id()`、`ordinal = next_native_ordinal(owner, kind)`、`at = default_insert_index(chain)?`、`insert_device(chain, at, Native(new_added(kind, id, ordinal)))`、id を返す → `open_panel` なら `open_rack_panels.insert(Device(id))` → `set_device_selection(vec![id])`、`device_anchor = Some(id)` |
