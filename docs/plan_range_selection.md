...
| オートメーションクリップの**名前帯** | 左ドラッグ | クリップ移動 (r.md #109: トラック行のクリップと同じ規約。点 / 線の当たりはこれより先に効く)。**範囲が掛かっているクリップなら範囲移動** (トラック行のヘッダと同じ `start_range_move`。範囲内のトラッククリップ + automation クリップが一緒に動き、ゴーストも両方描く)。掛かっていなければ単独移動 (lane 跨ぎ drop 付き) |
