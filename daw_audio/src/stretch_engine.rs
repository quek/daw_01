...
impl StretchEngine {
    ...
    /// このエンジンが出力位置より先に読む入力の量 (サンプル)。
    ///
    /// 出力のレイテンシは 0 に見せている (先読みで吸収) が、その代償として
    /// **素材の立ち上がりはこの幅だけ手前からスペクトルに現れる**
    /// (解析窓が先の入力を既に含むため)。 無音区間を検証する側 (r.md #41 の
    /// `wave_span_binding_tests`) が「境界のにじみ」 を除外する幅として使う。
    /// preset が変わっても追随するよう、値ではなくここから引くこと。
    /// 再生には使わない (= 検証専用) ので `cfg(test)`。
    #[cfg(test)]
