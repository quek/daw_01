...
//!
//! **記録側と比較側が同じこのコードを使う。** 最初の記録は旧 `mixer::channel_strip` の tests にあった
...
//! 消えるので、ここが旧型を引くと比較側がビルドできなくなる。音を意図して変えたときは
//! `native_dsp::tests::rerecord_golden` が今の実装で窓を取り直す (いつ・なぜ取り直したかはファイル先頭の
//! `記録:` 行)。
//!
