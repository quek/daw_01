...
impl Song {
    /// トラックが **実効的に有効か** (= 自分と祖先 group が全部 `enabled`、祖先の走査は `model::lineage`)。
    /// 無効化の判定の唯一の口 — 音声 / 映像 / host 同期 / 表示が全部ここを読む。master (`MASTER_TRACK_ID`) は
    /// 無効化できないので常に `true`、存在しないトラックは `false`。
    #[must_use]
    pub fn track_effectively_enabled(&self, track_id: u32) -> bool {
        if track_id == MASTER_TRACK_ID {
            return true;
        }
        self.lineage_all(track_id, |t| t.enabled)
    }
    ...
    pub fn executable_mask(&self, holds: impl Fn(&PluginInstance) -> bool) -> Vec<bool> {
        self.lineage_mask(|t| t.enabled && !plugins(&t.devices).any(&holds))
    }
