...
// ---- r.md #129 S0: v38 fixture (strip → native device の migration の正解) ----

/// `common/tests/fixtures/v38_strips.daw` (+ 同じ場所の `samples/v38_strips_loop.wav`) を
/// HEAD (v38) の型と保存経路 (`project::save`) で書き出す。
///
/// 組み込み Comp / EQ を `Device::Native` に作り直すと `Track.strip` / `Song.master_strip` /
/// `TrackBuiltinParam::Strip*` / `MasterStripParam` は型ごと消えるので、旧形式のファイルは
/// ここで HEAD に書かせたものだけが正解になる。読み手は `docs/plan_rack_native_devices.md`
/// §15.2 F-C10 / F-C11 / F-G3 と §15.3 T13 / T16。
///
/// 中身 (id はすべて固定):
/// - **track 1 "Lead"** (通常トラック): strip の comp / eq とも ON (SC 150Hz、HP ON、HF Bell、
///   LP は OFF で周波数だけ既定と違う)。devices = `[synth 1, Parallel 2 {chain 3: [chorus 4],
///   chain 5: []}, delay 6]`。音声クリップ 4 個 (拍 0..4)。Return への post-fader send。
///   レーン: 1 `StripEq{Hmf,Gain}` (クリップの点あり) / 2 `StripEq{Hp,Gain}` (実在しない組) /
///   3 routing 1 の深さ / 4 Parallel 内 plugin 4 の param / 5 chain 5 の `ChainGain` /
///   6 routing 2 の深さ。routing: 1 `StripComp{Ratio}` / 2 `StripEq{Lf,Q}` (実在しない組)。
/// - **track 2 "GWI"** (group-with-instrument): strip ON (Leveler)。devices = `[drum 7 (aux out 1 →
///   track 3), glue 8]` なので `paraout_split_device == Some(1)`。音声クリップ 2 個。
///   レーン: 1 `StripCompOn` / 2 `StripComp{Threshold}` / 3 `StripEqOn`。
/// - **track 3 "GWI Child"** (track 2 の子): strip は comp / eq とも OFF だが値は既定と違う。
///   devices が空 = JSON に `devices` キーが無い。
/// - **track 4 "Return"** (Lead の send 先): comp OFF、eq ON (HP / LP)。`devices` キー無し。
///   レーン 1 はトラックに紛れた `MasterStrip(EqOn)`。
/// - **master**: strip 全 ON (Release Auto)。`master_fx_chain = [maximizer 9, Parallel 10 {chain 11:
///   [exciter 12]}]`。song レーン: 1 `CompRatio` / 2 `LimiterOn` / 3 `LimiterCeiling` (クリップの点
///   あり) / 4 plugin 12 の param / 5 song 側に紛れた `TrackBuiltin(StripEqOn)` / 6 routing 3 の深さ。
///   song routing: 3 `MasterStrip(CompThreshold)`。
/// - **`ids.next_device_id = 2`** (実在する node の最大 id は Parallel の中の 12)。
///
/// `clip_contents` / `media` は HashMap なので、記録し直すとキーの並びだけは変わりうる (中身は同じ)。
#[test]
#[ignore = "r.md #129 S0: common/tests/fixtures/v38_strips.daw を記録する"]
fn record_v38_fixture() {
    let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures");
    std::fs::create_dir_all(dir.join("samples")).expect("mkdir fixtures/samples");
    write_v38_fixture_wav(&dir.join(V38_WAV_REL));
    let song = v38_fixture_song();
    let path = dir.join("v38_strips.daw");
    crate::project::save(&path, &song).expect("save v38 fixture");

    // 記録した性質が保存で潰れていないこと (migration テストはこれを前提に書かれる)。
    let raw: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&path).expect("read back")).expect("json");
    assert_eq!(raw["version"], 38);
    assert_eq!(raw["song"]["ids"]["next_device_id"], 2, "next_device_id の遅れが保存で消えた");
    let tracks = raw["song"]["tracks"].as_array().expect("tracks");
    assert!(tracks.iter().all(|t| t.get("strip").is_some()), "strip キーが無いトラックがある");
    assert!(tracks[2].get("devices").is_none() && tracks[3].get("devices").is_none());
    assert!(raw["song"].get("master_strip").is_some());
    let loaded = crate::project::load_project(&path).expect("HEAD で読める");
    for (a, b) in loaded.song.tracks.iter().zip(&song.tracks) {
        assert_eq!((a.id, a.strip), (b.id, b.strip));
        assert_eq!(a.automation_lanes.len(), b.automation_lanes.len(), "{} のレーンが HEAD の load で消えた", a.name);
    }
    assert_eq!(loaded.song.master_strip, song.master_strip);
    assert_eq!(loaded.song.song_lanes.len(), song.song_lanes.len());
}

const V38_WAV_REL: &str = "samples/v38_strips_loop.wav";
/// 120 BPM の 1 拍 (48 kHz)。
const V38_WAV_FRAMES: u32 = 24_000;

/// 0.5 s のステレオ PCM16。減衰する正弦 + ノイズ (L/R で周波数と減衰が違う)。ピークが 0dBFS 近くまで
/// 行くので、fixture の comp (threshold −18 dB 等) が実際に GR を出す。
fn write_v38_fixture_wav(path: &std::path::Path) {
    ...
    let to_i16 = |x: f64| (x.clamp(-1.0, 1.0) * 32_767.0).round() as i16;
    for i in 0..V38_WAV_FRAMES {
        let t = f64::from(i) / 48_000.0;
        let tau = std::f64::consts::TAU;
        let l = 0.8 * (tau * 110.0 * t).sin() * (-t / 0.12).exp() + 0.15 * noise();
        let r = 0.6 * (tau * 165.0 * t).sin() * (-t / 0.2).exp() + 0.15 * noise();
        w.write_sample(to_i16(l)).expect("write L");
        w.write_sample(to_i16(r)).expect("write R");
    }
    w.finalize().expect("finalize wav");
}

fn v38_fixture_song() -> Song {
    use crate::model::AutomationTarget as T;
    use crate::model::TrackBuiltinParam as B;
    let lane = |id: u32, target: AutomationTarget, default_value: f64| AutomationLane {
        id,
        ..AutomationLane::new(target, default_value)
    };
    let routing = |id: u32, target: AutomationTarget, source_id: u32, depth: f32, polarity: Polarity| {
        ModRouting { id, target, source_id, depth, polarity, enabled: true }
    };
    let auto_clip = |content_id: ContentId, length_beats: f64| AutomationClip {
        id: 1,
        start_beat: 0.0,
        length_beats,
        content_id,
    ...
    let audio_clips = |starts: &[f64]| -> Vec<Clip> {
        starts
            .iter()
            .zip(1..)
            .map(|(&start_beat, id)| Clip { id, start_beat, length_beats: 1.0, content_id: 1, ..Clip::default() })
            .collect()
    };
    ...
    let lead = Track {
        ...
        name: "Lead".into(),
        devices: vec![
            v38_plugin(1, "test.synth", false),
            Device::Parallel(Parallel {
                id: 2,
                name: "Parallel".into(),
                chains: vec![v38_chain(3, "Chain 1", vec![v38_plugin(4, "test.fx.chorus", true)]), v38_chain(5, "Chain 2", vec![])],
                bypassed: false,
                color: None,
                out_gain: 1.0,
                gain_match: false,
            ...
            v38_plugin(6, "test.fx.delay", true),
        ],
        strip: ChannelStrip {
            comp: CompSettings {
                on: true,
            ...
            eq: {
                ...
                EqSettings {
                    on: true,
                    hp: EqBandSettings { on: true, freq_hz: 60.0, ..d.hp },
                    lp: EqBandSettings { on: false, freq_hz: 15_000.0, ..d.lp },
                    lf: EqBandSettings { gain_db: 2.0, ..d.lf },
                    lmf: EqBandSettings { freq_hz: 350.0, gain_db: -3.0, q: 1.2, ..d.lmf },
                    hmf: EqBandSettings { freq_hz: 3_000.0, gain_db: 2.5, q: 0.9, ..d.hmf },
                    hf: EqBandSettings { freq_hz: 9_000.0, gain_db: 1.5, q: 0.8, bell: true, ..d.hf },
                }
            },
        },
        clips: audio_clips(&[0.0, 1.0, 2.0, 3.0]),
        next_clip_id: 5,
        sends: vec![Send { id: 1, dest_track_id: 4, gain: 0.5, mode: SendMode::PostFader, enabled: true }],
        next_send_id: 2,
        automation_lanes: vec![
            AutomationLane {
                clips: vec![auto_clip(2, 4.0)],
                next_clip_id: 2,
                ..lane(1, T::TrackBuiltin(B::StripEq { band: EqBand::Hmf, param: EqParam::Gain }), 2.5)
            },
            lane(2, T::TrackBuiltin(B::StripEq { band: EqBand::Hp, param: EqParam::Gain }), 3.0),
            lane(3, T::ModRoutingDepth { routing_id: 1 }, 0.35),
            lane(4, T::PluginParam { device_id: 4, param_id: 7, legacy_device_index: None }, 0.5),
            lane(5, T::TrackBuiltin(B::ChainGain { chain_id: 5 }), 0.8),
            lane(6, T::ModRoutingDepth { routing_id: 2 }, 0.1),
        ],
        next_lane_id: 7,
        mod_routings: vec![
            routing(1, T::TrackBuiltin(B::StripComp { param: CompParam::Ratio }), 1, 0.3, Polarity::Bipolar),
            routing(2, T::TrackBuiltin(B::StripEq { band: EqBand::Lf, param: EqParam::Q }), 1, 0.2, Polarity::Unipolar),
        ],
        ..Track::default()
    };
    ...
    let gwi = Track {
        ...
        devices: vec![
            Device::Plugin(PluginInstance {
                aux_outputs: vec![None, Some(AuxOutputRoute::to_track(3))],
                aux_output_count: 2,
                ..v38_plugin_instance(7, "test.drum_sampler", false)
            }),
            v38_plugin(8, "test.fx.bus_glue", true),
        ],
        strip: ChannelStrip {
            comp: CompSettings {
                on: true,
                mode: CompMode::Leveler,
                threshold_db: -20.0,
                ratio: 3.0,
                attack_ms: 12.0,
                release_ms: 300.0,
            ...
            eq: {
                ...
                EqSettings {
                    on: true,
                    lf: EqBandSettings { gain_db: -2.0, ..d.lf },
                    hf: EqBandSettings { gain_db: 3.0, ..d.hf },
                    ..d
                }
            },
        },
        clips: audio_clips(&[0.0, 2.0]),
        next_clip_id: 3,
        automation_lanes: vec![
            lane(1, T::TrackBuiltin(B::StripCompOn), 1.0),
            lane(2, T::TrackBuiltin(B::StripComp { param: CompParam::Threshold }), -20.0),
            lane(3, T::TrackBuiltin(B::StripEqOn), 1.0),
        ],
        next_lane_id: 4,
        ..Track::default()
    };
    ...
    let child = Track {
        ...
        strip: ChannelStrip {
            ...
            eq: {
                let d = EqSettings::default();
                EqSettings { on: false, lmf: EqBandSettings { gain_db: 4.0, q: 2.0, ..d.lmf }, ..d }
            },
        },
        ..Track::default()
    };
    ...
    let ret = Track {
        ...
        strip: ChannelStrip {
            ...
            eq: {
                ...
                EqSettings {
                    on: true,
                    hp: EqBandSettings { on: true, freq_hz: 200.0, ..d.hp },
                    lp: EqBandSettings { on: true, freq_hz: 8_000.0, ..d.lp },
                    ..d
                }
            },
        },
        automation_lanes: vec![lane(1, T::MasterStrip(MasterStripParam::EqOn), 1.0)],
        next_lane_id: 2,
        ..Track::default()
    };

    let points = |a: (f64, f64), b: (f64, f64)| {
        ClipContent::Automation(AutomationContent {
            points: vec![
                AutomationPoint { id: 1, time_beat: a.0, value: a.1, curve: AutomationCurve::Linear },
                AutomationPoint { id: 2, time_beat: b.0, value: b.1, curve: AutomationCurve::Linear },
            ],
            next_point_id: 3,
        })
    };
    let audio = ClipContent::Audio(AudioContent {
        events: vec![AudioEvent {
            ...
            event_length_beats: 1.0,
            source_start_frames: 0,
            source_end_frames: u64::from(V38_WAV_FRAMES),
            stretch_mode: StretchMode::Raw,
            ..AudioEvent::default()
        }],
        next_event_id: 2,
    });
    let lfo = |id: u32, owner_track_id: u32| ModSource {
        id,
        owner_track_id,
        color: ModSource::palette_color(id as usize - 1),
        kind: ModSourceKind::Lfo(LfoConfig::default()),
        enabled: true,
    };
    ...
    Song {
        ...
        time_sig: (4, 4),
        length_beats: 8.0,
        tracks: vec![lead, gwi, child, ret],
        ids: IdAllocators {
            next_track_id: 5,
            // 実在する最大 node id (12) より遅れている。
            next_device_id: 2,
            next_content_id: 4,
            ...
            next_song_lane_id: 7,
            next_section_id: 1,
            next_mod_source_id: 3,
            next_mod_routing_id: 4,
            next_scene_id: 1,
        },
        clip_contents: HashMap::from([(1, audio), (2, points((0.0, -4.0), (4.0, 6.0))), (3, points((0.0, -1.0), (8.0, -3.0)))]),
        media: MediaPools {
            audio_sources: HashMap::from([(
                1,
                AudioSource {
                    path: AudioSourcePath::ProjectRelative(PathBuf::from(V38_WAV_REL)),
                    sample_rate: 48_000,
                    channels: 2,
                    frames: u64::from(V38_WAV_FRAMES),
                    original_bpm: None,
                    root_key: None,
                },
            )]),
        ...
        song_lanes: vec![
            lane(1, T::MasterStrip(MasterStripParam::CompRatio), 2.0),
            lane(2, T::MasterStrip(MasterStripParam::LimiterOn), 1.0),
            AutomationLane {
                clips: vec![auto_clip(3, 8.0)],
                next_clip_id: 2,
                ..lane(3, T::MasterStrip(MasterStripParam::LimiterCeiling), -1.0)
            },
            lane(4, T::PluginParam { device_id: 12, param_id: 3, legacy_device_index: None }, 0.25),
            lane(5, T::TrackBuiltin(B::StripEqOn), 1.0),
            lane(6, T::ModRoutingDepth { routing_id: 3 }, 0.15),
        ],
        master_fx_chain: vec![
            v38_plugin(9, "test.fx.maximizer", true),
            Device::Parallel(Parallel {
                id: 10,
                name: "Parallel".into(),
                chains: vec![v38_chain(11, "Chain 1", vec![v38_plugin(12, "test.fx.exciter", true)])],
                bypassed: false,
                color: None,
                out_gain: 1.0,
                gain_match: false,
        ...
        master_strip: MasterStrip {
            comp: MasterCompSettings {
                on: true,
                threshold_db: -12.0,
                ratio: MasterRatio::R4,
                attack: MasterAttack::A10,
                release: MasterRelease::Auto,
                makeup_db: 2.0,
            },
            eq: MasterEqSettings { on: true, low_db: 1.5, lomid_db: -1.0, high_db: 2.0 },
            limiter: MasterLimiterSettings { on: true, ceiling_db: -0.5 },
        },
        project_id: 0x5633_3853_5452_4950,
        mod_sources: vec![lfo(1, 1), lfo(2, MASTER_TRACK_ID)],
        song_mod_routings: vec![routing(
            3,
            T::MasterStrip(MasterStripParam::CompThreshold),
            2,
            -0.2,
            Polarity::Unipolar,
        )],
...
fn v38_plugin_instance(id: u64, plugin_id: &str, effect: bool) -> PluginInstance {
    let ports = crate::port_config::PortConfig {
        has_note_input: !effect,
        has_audio_input: effect,
...
fn v38_plugin(id: u64, plugin_id: &str, effect: bool) -> Device {
    Device::Plugin(v38_plugin_instance(id, plugin_id, effect))
}

fn v38_chain(id: u64, name: &str, devices: Vec<Device>) -> ParallelChain {
    ParallelChain { id, name: name.into(), color: None, gain: 1.0, pan: 0.0, muted: false, solo: false, devices }
}
