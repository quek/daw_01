...
/// シナリオは「track 上に追加した Text クリップの default 「Title」 64 px white が
...
/// 通常 smoke と違い fixture を取らず、 内部で track 追加 + `AddTextClipAt` を
/// 発火して default "Title" clip を生成 → preview を開いて Play → 1.5s 後に
...
pub fn spawn_text_overlay_orchestrator(proxy: EventLoopProxy<AppEvent>) {
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // Phase 2: 普通の track を 1 本作り (AddInstrumentTrack)、 その
            // track の beat 0 に Text クリップを追加する。 旧 AddTextClip
            // (新規 track 自動生成版) は廃止され、 text は「text トラック」 では
            // なく他 clip と同じくタイムライン上で生成する
            // (docs/plan_text_clip_creation.md)。 fresh boot の Song::default は
            // tracks 空 + next_track_id=1 なので最初の AddInstrumentTrack は
            // id=1 を採番する (= AddTextClipAt の target が決定論的)。 default
            // TextEvent は text="Title" / 64px / 中央横帯 / shadow.a=0.5 で
            // gui_01 effects path も exercise する。 handler は GUI thread で
            // 同期実行。
            ...
            if proxy
                ...
                .is_err()
            {
                fail("event loop closed before AddTextClipAt could be sent");
            }
