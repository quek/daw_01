...
pub(super) fn build(app: &AppData, area: Rect) -> BuiltArrangement {
    ...
    // r.md #130: 移調に実効的に追従するか (追従しないトラックはヘッダに印)。
    let follows = app.cur.song_doc.song().follows_transpose_mask();
    let tracks: Vec<ArrangementTrack> = app
        ...
        .map(|(track_idx, t)| ArrangementTrack {
            ...
            no_transpose_mark: app.track_shows_no_transpose_mark(t, follows[track_idx]),
        })
        .collect();
