...
// `draw_loop_band` は M14 Phase 69 (#041) で `daw_ui_core::widgets::ruler_ops::draw_loop_band` に extract
...
// ============================================================
    ...
    #[allow(clippy::too_many_arguments, clippy::too_many_lines)]
pub fn arrangement<'a, M: ?Sized + 'static, F>(
        ui: &mut Ui<'a, M>,
        id: impl Hash,
        rect: Rect,
        tracks: &[ArrangementTrack],
        ...
        F: Fn(ArrangementEditRequest) -> Edit<M> + Clone + Send + Sync + 'static,
    {
            ...
            if let Some((ax, ay)) = axes.or_else(|| marquee_active.then_some((true, true))) {
                ...
                if dx != 0.0 || dy != 0.0 {
                    ...
                    if dx != 0.0 && beat_per_px > 1e-6 {
                        ...
                        if adx != 0.0 {
                            applied_beat_px = adx;
                            ui.push_edit(make_edit(ArrangementEditRequest::SetScrollX(
                                new_start,
                            )));
                        }
                    }
                    if dy != 0.0 {
                        ...
                        if ady != 0.0 {
                            ...
                            ui.push_edit(make_edit(ArrangementEditRequest::SetTrackTop(
                                new_top,
                            )));
                        }
                    }
            ...
            && !pointer.primary_just_released
        {
            ...
            if let Some((lane, prev, next)) = emit {
                ui.push_edit(make_edit(ArrangementEditRequest::SetLaneHeight {
                    lane,
                    prev,
                    next,
                }));
            ...
            && !pointer.primary_just_released
        {
            ...
            if let Some((track, prev, next)) = row_emit {
                ui.push_edit(make_edit(ArrangementEditRequest::SetSingleTrackRowH {
                    track,
                    prev,
                    next,
                }));
            ...
            && !pointer.primary_just_released
        {
            ...
            if let Some((prev, next)) = header_emit {
                ui.push_edit(make_edit(ArrangementEditRequest::SetHeaderW { prev, next }));
            }
        }
            ...
            && !pointer.primary_just_released
        {
            ...
            if let Some((track, prev, next)) = volume_emit {
                ui.push_edit(make_edit(ArrangementEditRequest::SetTrackVolume {
                    track,
                    prev,
                    next,
                }));
        ...
        if ui.take_shortcut("delete") {
            if !selected_clips.is_empty() {
                ui.push_edit(make_edit(ArrangementEditRequest::DeleteClips(
                    selected_clips.to_vec(),
                )));
            } else if let Some(&tid) = selected_tracks.first() {
                ui.push_edit(make_edit(ArrangementEditRequest::DeleteTrack(tid)));
            }
        }
        ...
        if let Some(nd) = clip_drag_release {
            ...
            match nd.kind {
                ...
                ClipDragKind::ResizeRight => {
                    ...
                    if !deltas.is_empty() {
                        ui.push_edit(make_edit(ArrangementEditRequest::ResizeClips(deltas)));
                    }
                }
                ClipDragKind::ResizeLeft => {
                    let mut deltas: Vec<ResizeClipDelta> = Vec::new();
            ...
            && let Some(out) = compute_audio_drag_outcome(&ad, beat_per_px, style)
        {
            match out {
                AudioDragOutcome::Gain { next_db } => {
                    ...
                    ui.push_edit(make_edit(ArrangementEditRequest::SetClipGainDb(vec![
                        delta,
                    ])));
                }
                AudioDragOutcome::FadeLength { edge, next_beats } => {
                    ...
                    ui.push_edit(make_edit(ArrangementEditRequest::SetClipFade(vec![delta])));
                }
                AudioDragOutcome::FadeCurve { edge, next_curve } => {
                    let delta = ClipFadeCurveDelta { key: ad.key, edge, next_curve };
                    ui.push_edit(make_edit(ArrangementEditRequest::SetClipFadeCurve(vec![
                        delta,
                    ])));
                }
            }
        ...
        if let Some(ad) = point_drag_release {
            ...
            if dist >= 4.0 {
                ...
                if !deltas.is_empty() {
                    ui.push_edit(make_edit(ArrangementEditRequest::MoveAutomationPoints(
                        deltas,
                    )));
                }
            } else if !ad.last_alt {
        ...
        {
            ui.push_edit(make_edit(ArrangementEditRequest::SetAutomationCurveParam {
                point: cd.point,
                kind: cd.kind,
        ...
        if let Some(ls) = automation_lasso_release {
            ...
            if clip_next != clip_prev {
                ui.push_edit(make_edit(ArrangementEditRequest::SelectAutomationClips {
                    prev: clip_prev,
                    next: clip_next,
                }));
                response.selection_changed = true;
            ...
            && lanes.contains(cx, cy)
        {
            ...
            if prev != next {
                ui.push_edit(make_edit(ArrangementEditRequest::SelectClips { prev, next }));
                response.selection_changed = true;
            }
        ...
        {
            ui.push_edit(make_edit(ArrangementEditRequest::SelectClips {
        ...
        if let Some(ld) = loop_drag_release {
            ...
            let (start, end) =
                compute_loop_drag_endpoints(&ld, cur_beat, &view.snap, zoom_x_px_per_beat);
            ui.push_edit(make_edit(ArrangementEditRequest::SetLoopRange { start, end }));
        }
        ...
        if let Some((src_tracks, parent, anchor_after)) = pending_drop {
            ui.push_edit(make_edit(ArrangementEditRequest::SetTrackParent {
                tracks: src_tracks,
                parent,
                anchor_after,
            }));
        ...
        if scroll.1.abs() > 0.0 || scroll.0.abs() > 0.0 {
            ...
            if pointer.modifiers.ctrl && over_lanes {
                ...
                ui.push_edit(make_edit(ArrangementEditRequest::SetZoomX(new_zoom)));
                    ...
                    && view.track_row_h > 0.0
                {
                    ...
                    ui.push_edit(make_edit(ArrangementEditRequest::SetTrackTop(new_top)));
                }
                ui.push_edit(make_edit(ArrangementEditRequest::SetTrackRowH(new_h)));
            ...
            match sd.kind {
                SectionGesture::Create => {
                    ...
                    if dist >= 4.0 {
                        ...
                        if len >= SECTION_MIN_LEN_BEATS {
                            ui.push_edit(make_edit(ArrangementEditRequest::CreateSection {
                                start,
                                len,
                            }));
                        }
                ...
                SectionGesture::Move => {
                    if dist < 4.0 {
                        ...
                        ui.push_edit(make_edit(ArrangementEditRequest::SelectSection {
                            id: sd.section_id,
                            modifier,
                        }));
                        ui.push_edit(make_edit(ArrangementEditRequest::SetPlayheadBeat(
                            sd.anchor_start.max(0.0),
                        )));
                    } else if sd.last_ctrl {
                        let next_start = (sd.anchor_start + delta).max(0.0);
                        ui.push_edit(make_edit(ArrangementEditRequest::DuplicateSection {
                            id: sd.section_id,
                            dest_start: next_start,
                        }));
                    } else {
                        let next_start = (sd.anchor_start + delta).max(0.0);
                        ui.push_edit(make_edit(ArrangementEditRequest::MoveSection {
                            id: sd.section_id,
                ...
                SectionGesture::ResizeLeft => {
                    ...
                    ui.push_edit(make_edit(ArrangementEditRequest::ResizeSection {
                        id: sd.section_id,
                        prev_start: sd.anchor_start,
            ...
            && let Some((cx, cy)) = ui.take_double_click_in_rect(arranger_rect)
        {
            if let Some(sid) = section_at_inrect(sections, arranger_rect, view, cx, cy) {
                ui.push_edit(make_edit(ArrangementEditRequest::BeginRenameSection(sid)));
            } else {
        ...
        {
            ui.push_edit(make_edit(ArrangementEditRequest::SecondaryClickSection {
                id: sid,
                pos: (cx, cy),
            }));
        }
        ...
        if let Some((cx, cy)) = ui.take_double_click_in_rect(lanes) {
            if let Some((hit_key, _)) =
                clip_hit(&visible_tracks, &press_tops, view, lanes, cx, cy, style.resize_handle_px)
            {
                ui.push_edit(make_edit(ArrangementEditRequest::DoubleClickClip(hit_key)));
                ...
                ui.push_edit(make_edit(ArrangementEditRequest::DoubleClickAutomationPoint(
                    pt_key,
                )));
                    ...
                    automation_clip_at(track_id, lane, body_rect, view, style, cx, cy)
                {
                    ...
                    ui.push_edit(make_edit(ArrangementEditRequest::AddAutomationPoint {
                        clip: clip_key,
                    ...
                    if !on_existing_clip {
                        ...
                        ui.push_edit(make_edit(ArrangementEditRequest::CreateAutomationClip {
                            lane: lane_key,
                ...
                && t.id != MASTER_TRACK_ID
            {
                ...
                if cy < track_row_top + effective_track_row_h(t, view.track_row_h) {
                    ...
                    ui.push_edit(make_edit(ArrangementEditRequest::DoubleClickEmpty {
                        track: t.id,
                        beat,
                    }));
                }
        ...
        if let Some((cx, cy)) = ui.take_secondary_press_in_rect(lanes) {
                ...
                && t.id != MASTER_TRACK_ID
            {
                ...
                if cy < track_row_top + effective_track_row_h(t, view.track_row_h) {
                    ...
                    ui.push_edit(make_edit(ArrangementEditRequest::SecondaryClickEmpty {
                        track: t.id,
                        beat,
                        pos: (cx, cy),
                    }));
        ...
        if header_w > 0.0 {
            for (visible_i, &i) in visible_idx_for_headers.iter().enumerate() {
                ...
                if let Some((dcx, dcy)) = ui.take_double_click_in_rect(rename_hit) {
                    ...
                    if !in_subzone {
                        ui.push_edit(make_edit(ArrangementEditRequest::BeginRenameTrack(track_id)));
                    }
                }
        ...
        if let Some(tid) = disclosure_clicked {
            ui.push_edit(make_edit(ArrangementEditRequest::ToggleGroupCollapsed(tid)));
        ...
        if let Some(tid) = clicked_track_for_select {
            ...
            if prev_sorted != next_sorted {
                ui.push_edit(make_edit(ArrangementEditRequest::SelectTrack {
                    prev: prev_v,
                    next,
                    modifier,
                }));
...
mod tests {
    ...
    fn automation_clip_edge_hover_sets_ew_resize_cursor() {
        ...
        let cursor_at = |pos: (f32, f32)| -> Vec<CursorIcon> {
            ...
            let mut host: UiHost<()> = UiHost::no_redraw();
            host.set_cursor_sink(Box::new(move |c| cc.lock().unwrap().push(c)));
            let mut scene = Scene::new();
            let screen = PhysicalSize { width: 800, height: 400 };
            let input = FrameInput {
                pointer: PointerFrame { pos: Some(pos), ..PointerFrame::default() },
    ...
    fn ruler_plain_click_applies_snap_when_active() {
        ...
        use daw_ui_core::input::{FrameInput, PointerFrame};
        use daw_ui_core::ui::UiHost;
        use daw_ui_core::snap::{SnapConfig, SnapMode};
