...
impl AppData {
    ...
    /// r.md #52: count-in の残り小節数 (1 起算)。 count-in 中でなければ `None`。
    ///
    /// transport の Rec ボタンが `●` の代わりにこの数字を出す (2 → 1 と減り、
    /// engine が `recording_live` を上げたら赤い `●` に戻る)。 端数の小節は「まだ
    /// 残っている」ので切り上げる (= 1 小節目の途中なら 1)。
    ///
    /// 分母は [`Self::count_in_samples`] を割り戻す (= 送った preroll と同じ式)。
    /// 総量を別フィールドに複製しないのは、それが engine の持つ値の二重管理に
    ...
    pub(crate) fn count_in_remaining_bars(&self) -> Option<u64> {
        ...
        let per_bar = self.count_in_samples() / bars;
        if per_bar == 0 {
            return None;
        }
