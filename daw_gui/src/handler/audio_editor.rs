...
impl AppData {
    ...
    pub(crate) fn duplicate_audio_editor_event(&mut self) {
        ...
        let Some(Some(insert_at)) = self.edit_song(|song| {
            ...
            // 複製した take は元の take の Melodyne の編集から始める (id / take を採り直す前に記録する)。
            new_event.record_copied_from(song.project_id, content_id);
            ...
            // 複製は元とは別の take (以後は ARA の編集を共有しない、`AudioContent::adopt_events` と同じ規則)。
            new_event.take_id = 0;
            let insert_at = idx + 1;
