...
impl AppData {
    ...
    pub(crate) fn begin_loudness_analysis(&mut self, range: Option<(f64, f64)>) {
        ...
        self.cur.loudness.generation = self.cur.loudness.generation.wrapping_add(1);
        self.cur.loudness.phase = LoudnessPhase::AwaitingReinit { range };
        self.cur.loudness.progress_at = Some(std::time::Instant::now());
    ...
    pub(crate) fn fire_pending_loudness_analysis(&mut self) -> bool {
        let LoudnessPhase::AwaitingReinit { range } = self.cur.loudness.phase else {
            return false;
        };
        self.cur.loudness.phase = LoudnessPhase::Running;
        self.cur.loudness.running_generation = self.cur.loudness.generation;
        self.cur.loudness.progress_at = Some(std::time::Instant::now());
        self.send_audio(AudioCommand::AnalyzeLoudness { project: self.pk(), range });
        true
    }
    ...
    pub(crate) fn cancel_loudness_analysis(&mut self) {
        match self.cur.loudness.phase {
            ...
            LoudnessPhase::Running => {
                self.cur.loudness.phase = LoudnessPhase::Cancelling;
    ...
    pub(crate) fn abort_loudness_analysis(&mut self, reason: String) {
        ...
        self.cur.loudness.phase = LoudnessPhase::Idle;
        self.cur.loudness.progress_at = None;
    ...
    pub(crate) fn on_loudness_progress(&mut self, report: Box<LoudnessReport>) {
        ...
        self.cur.loudness.progress_at = Some(std::time::Instant::now());
        self.cur.loudness.report = Some(report);
    }
    ...
    pub(crate) fn on_loudness_complete(
        ...
        if let Some(err) = error {
            self.cur.loudness.error = Some(err.clone());
            self.cur.loudness.report = None;
        ...
        if let Some(r) = report {
            ...
            self.cur.loudness.report = Some(r);
            // 「この値がどの Song 状態のものか」を epoch で固定する。以後 epoch が
            // 進めば (編集 / undo / redo / プロジェクト差し替え) 自動的に古くなる。
            self.cur.loudness.report_epoch = self.cur.song_doc.edit_epoch();
    ...
    pub fn loudness_report_stale(&self) -> bool {
        self.cur.loudness.report.is_some() && self.cur.loudness.report_epoch != self.cur.song_doc.edit_epoch()
    }
    ...
    pub(crate) fn seek_to_loudness_position(&mut self, secs: f32) {
        let Some(report) = self.cur.loudness.report.as_ref() else {
            return;
        };
        let frame = report.song_frame_at(secs);
        let beat =
            common::automation::samples_to_beats(self.cur.song_doc.song(), report.sample_rate, frame);
