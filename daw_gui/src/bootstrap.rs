...
/// plugin_host worker は各コアの 1 本目に制限 = `common::cpu_topology`。GUI あり・File→Open → Space で
/// 再生し、master が鳴っていることを毎窓で確認。1 回 ≈ 3,000 buffer):
///
/// ```text
/// runners  xrun/秒 (各回)
///      16  3.66  2.61
///      24  1.41  1.83  1.24  1.63  0.52   <- 最良
///      30  2.45  2.78
/// ```
///
/// 16 に絞ると worker の CPU が依頼の合間に空く (runner は依頼の前後にも仕事がある)、30 に増やすと
/// 取り合いが増える。24 = **論理コアの 3/4** を既定にする。
///
