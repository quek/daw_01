...
fn invert_effect_transforms_colors() {
    ...
    let out = engine.apply_chain(&mut r, src, W, H, &[invert(1.0)]);
...
fn invert_amount_zero_is_passthrough() {
    ...
    let mut engine = VideoFxEngine::new();
    let out = engine.apply_chain(&mut r, src, W, H, &[invert(0.0)]);
...
fn two_pass_ping_pong_restores_original() {
    ...
    let mut engine = VideoFxEngine::new();
    let out = engine.apply_chain(&mut r, src, W, H, &[invert(1.0), invert(1.0)]);
...
fn two_same_size_layers_do_not_collide() {
    ...
    let out_red = engine.apply_chain(&mut r, src_red, W, H, &[invert(1.0)]);
    let out_blue = engine.apply_chain(&mut r, src_blue, W, H, &[invert(0.0)]);
...
fn empty_chain_returns_src() {
    ...
    let src = r.create_texture(W, H);
    let mut engine = VideoFxEngine::new();
    let out = engine.apply_chain(&mut r, src, W, H, &[]);
