...
//! Open / Open Recent) をしようとしたときの保存確認ガード (FIXME #63) の回帰テスト。
//!
//! 検証する状態機械 (`AppData`):
...
fn not_dirty_close_quits_immediately() {
    ...
    assert!(app.dirty_guard.is_none(), "no confirm modal when clean");
}
...
fn discard_quits_without_saving() {
    ...
    assert!(app.dirty_guard.is_none(), "modal closed after discard");
...
fn cancel_keeps_app_running() {
    ...
    assert!(app.dirty_guard.is_none(), "modal closed after cancel");
...
fn save_without_plugins_saves_synchronously_then_quits() {
    ...
    assert!(app.dirty_guard.is_none(), "modal closed");
    assert!(app.guard_after_save.is_none(), "no async wait needed");
}
...
fn save_with_plugins_waits_for_states_then_quits() {
    ...
    assert!(app.guard_after_save.is_none(), "async-quit intent cleared");
}

// ---------------------------------------------------------------------------
// FIXME #63: New / Open / Open Recent も同じガードを通る。
// ---------------------------------------------------------------------------

#[test]
fn clean_new_runs_immediately() {
    let (mut app, _rx) = build_app();
    app.file_path = Some(std::path::PathBuf::from("C:/some/proj.daw"));
    app.is_dirty = false;

    app.handle_event(AppEvent::New);

    assert!(app.dirty_guard.is_none(), "clean New does not open modal");
    assert!(
        app.file_path.is_none(),
        "New cleared file_path (action_new ran)"
    );
    assert!(!app.is_dirty, "fresh project is clean");
}
...
fn dirty_new_opens_guard_modal() {
    ...
    assert_eq!(app.file_path, Some(path), "project not discarded yet");
    assert!(app.is_dirty, "still dirty until resolved");
}
...
fn dirty_new_discard_runs_new() {
    ...
    assert!(app.dirty_guard.is_none(), "modal closed after discard");
    assert!(app.file_path.is_none(), "New ran (file_path cleared)");
    assert!(!app.is_dirty, "fresh project is clean");
}
...
fn dirty_new_cancel_keeps_project() {
    ...
    assert!(app.dirty_guard.is_none(), "modal closed after cancel");
    assert_eq!(app.file_path, Some(path), "project kept (New cancelled)");
    assert!(app.is_dirty, "cancel does not discard or save");
}
...
fn dirty_new_save_then_runs_new() {
    ...
    assert!(app.dirty_guard.is_none(), "modal closed");
    assert!(app.guard_after_save.is_none(), "no async wait needed");
    assert!(app.file_path.is_none(), "New ran after save (file_path cleared)");
    assert!(!app.is_dirty, "fresh project is clean");
}
...
fn second_guarded_action_ignored_while_modal_open() {
    ...
    app.is_dirty = true;
    app.handle_event(AppEvent::New);
    assert_eq!(app.dirty_guard, Some(DirtyGuardAction::New));
...
// ---------------------------------------------------------------------------
// FIXME #63 レビュー指摘の回帰テスト (非同期保存との競合 / クラッシュ leak)。
// ---------------------------------------------------------------------------
...
fn new_during_in_flight_save_preserves_old_project() {
    ...
    let real_track_count = app.song.tracks.len();
    app.file_path = Some(path.clone());
    app.is_dirty = true;
    ...
    app.handle_event(AppEvent::New);
    assert!(app.dirty_guard.is_none(), "no modal opened mid-save");
    assert_eq!(
    ...
    assert!(app.file_path.is_none(), "New ran after the save completed");
    assert!(app.guard_pending_action.is_none(), "deferred action consumed");
    assert!(app.guard_after_save.is_none(), "no save-after intent left");
    assert!(
        app.pending_state_queue.is_empty(),
...
fn open_recent_during_deferred_edit_defers_then_reevaluates() {
    ...
    let extra = app.song.tracks[0].clone();
    app.song.tracks.push(extra);
    let target_idx = (app.song.tracks.len() - 1) as u32;
    ...
    // round-trip 中に Open Recent。 self.song を差し替えず、 モーダルも出さず保留する。
    let target = std::path::PathBuf::from("C:/some/other.daw");
    app.handle_event(AppEvent::OpenRecent(target.clone()));
    assert!(
        app.dirty_guard.is_none(),
    ...
    app.handle_event(AppEvent::AllStatesReceived(Vec::new()));
    assert!(app.guard_pending_action.is_none(), "deferred guard consumed");
    assert_eq!(
        app.dirty_guard,
        Some(DirtyGuardAction::OpenPath(target)),
...
fn plugin_host_disconnect_unblocks_dirty_guard() {
    ...
    app.guard_pending_action = Some(DirtyGuardAction::New);
...
fn discard_then_reopen_same_file_has_no_recovery_modal() {
    ...
    common::project::save(&proj, &app.song).expect("write project file");
    app.file_path = Some(proj.clone());
    app.is_dirty = true;
    // 未保存変更を写した sidecar autosave を用意 (.daw より後に書くので newer)。
    let sidecar = common::recovery::sidecar_for(&proj);
    common::project::save(&sidecar, &app.song).expect("write sidecar autosave");
