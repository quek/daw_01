...
fn set_arrange_layout(
    ...
    app.cur.peph.last_arrange_lanes_size = (lanes_w, lanes_h);
    let mut y = 0.0;
    app.cur.peph.last_arrange_rows = rows
        .iter()
        .map(|&(key, height)| {
...
fn z_横ズームは選択された_automation_clip_を対象にする() {
    ...
    app.cur.selection.selected_automation_clips = vec![AutomationClipKey {
        track: track_id,
        lane: 1,
        clip: 1,
    }];
    ...
    assert_eq!(app.cur.view.arrange_scroll_beat, 7.5);
    assert_eq!(app.cur.view.arrange_zoom_x, 160.0);
}
...
fn z_縦ズームは_automation_clip_の_track_を対象にする() {
    ...
    assert_eq!(app.cur.view.arrange_track_row_h, 600.0);
    assert_eq!(app.cur.view.arrange_track_top, 600.0);
}
...
fn z_縦ズームは_master_行の_automation_clip_を行_0_として扱う() {
    ...
    assert_eq!(app.cur.view.arrange_track_row_h, 600.0);
    assert_eq!(app.cur.view.arrange_track_top, 0.0);
}
...
fn z_は対象面ごとに片方の選択だけを_framing_する() {
    ...
    add_track_automation_clip(&mut app, 0, 1, 1, 8.0, 4.0);
    app.cur.selection.selected_automation_clips = vec![AutomationClipKey { track: track_id, lane: 1, clip: 1 }];
    set_default_layout(&mut app, track_id);
    ...
    assert_eq!(app.cur.view.arrange_scroll_beat, 0.0);
    assert_eq!(app.cur.view.arrange_zoom_x, 160.0);
...
fn z_縦ズームは選択_automation_clip_のレーンを画面いっぱいに拡大する() {
    ...
    assert_eq!(app.cur.view.automation_lane_row_overrides.get(&lane_key).copied(), Some(600));
    assert_eq!(app.cur.view.arrange_track_top, 250.0);
}
...
fn lane_拡大は_fresh_zoom_で破棄_fit_で行高に_scale_される() {
    ...
    app.handle_event(AppEvent::ZoomArrangeToSelectedClip { automation: false });
    assert!(
        app.cur.view.automation_lane_row_overrides.is_empty(),
        "fresh zoom should drop the lane-fill override, got {:?}",
        app.cur.view.automation_lane_row_overrides
    );
    ...
    assert_eq!(
        app.cur.view.automation_lane_row_overrides.get(&lane_key).copied(),
        Some(200),
        "fit should scale the lane to the fitted row height (not leave it at 600)",
    );
    assert_eq!(app.cur.view.arrange_track_row_h, 200.0);
}
...
fn fit_が縮めたレーン高は_fresh_zoom_で消えない() {
    ...
    app.handle_event(AppEvent::FitArrangeToContent);
    assert_eq!(app.cur.view.automation_lane_row_overrides.get(&lane_key).copied(), Some(200));
...
fn z_縦ズームは展開レーンを含めて行位置と行高を決める() {
    ...
    for (sel, want_row_h, want_top) in cases {
        let (mut app, _rx) = build_app();
        let track_a = app.cur.song_doc.song().tracks[0].id;
        app.edit_song(|song| {
            song.tracks[0].clips.push(Clip {
                id: 1,
                start_beat: 0.0,
        ...
        assert_eq!(app.cur.view.arrange_track_row_h, want_row_h, "sel={sel}");
        assert_eq!(app.cur.view.arrange_track_top, want_top, "sel={sel}");
    }
}
...
fn fit_は全行の高さ合計を_lanes_高さに一致させる() {
    ...
    for (lanes_h, track_rows, lane_rows) in cases {
        ...
        let row_h = app.cur.view.arrange_track_row_h;
        let total: f32 = row_h * track_rows as f32
            + app
                ...
                .sum::<f32>();
        assert_eq!(
            app.cur.view.automation_lane_row_overrides.len(),
            lane_rows,
...
fn z_は別_clip_を選び直すと新しい選択へ横ズームし直す() {
    ...
    app.cur.selection.selected_automation_clips = vec![AutomationClipKey { track: track_id, lane: 1, clip: 2 }];
    app.handle_event(AppEvent::ZoomArrangeToSelectedClip { automation: true });
    // 20..24 の横ズーム: pad 0.5 → scroll 19.5。 縦ズーム (no-op on scroll) のままなら 7.5 で落ちる。
    assert_eq!(app.cur.view.arrange_scroll_beat, 19.5);
    assert_eq!(app.cur.view.arrange_zoom_x, 160.0);
}
...
fn r_loop_は選択された_automation_clip_を対象にする() {
    ...
    assert_eq!(app.cur.transport.loop_region.start_beat, 8.0);
    assert_eq!(app.cur.transport.loop_region.end_beat, 12.0);
    assert!(app.cur.transport.loop_region.enabled);
}
