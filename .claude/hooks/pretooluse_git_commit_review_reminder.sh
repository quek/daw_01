...
# Match `git commit`, `git -C <path> commit`, `git -C "<quoted path>" commit`,
# etc. in the tool_input.command substring. The `-C` path arm accepts a quoted
# ("[^"]+") OR an unquoted ([^ "]+) path — worktree sessions routinely commit
# via `git -C "<abs worktree path>" commit`, and an earlier `[^ "]+`-only regex
# silently missed the quoted form (so the reminder never fired there). A
# false-positive on the substring is acceptable — an extra reminder is cheap, a
# missed one is not.
